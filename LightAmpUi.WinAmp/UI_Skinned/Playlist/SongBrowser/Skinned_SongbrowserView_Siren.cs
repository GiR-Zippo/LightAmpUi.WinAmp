...
namespace Skin.Ui
{
    ...
    public partial class Skinned_SongbrowserView : Window
    {
        ...
        {
            var filename = PlaylistContainer.SelectedItems.OfType<KeyValuePair<string, string>>().First().Key;
            if (!File.Exists(filename) || filename == null)
                return;

            BmpSong song = BmpSong.OpenFile(filename).Result;
            if (song == null)
                return;
            scrollpos = 0;
            lasttime = 0;

            this.WriteSongTitle("haha"); // song.Title);
            _ = BmpSiren.Instance.Load(song);
        }
